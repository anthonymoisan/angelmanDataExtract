...
@ratelimit(3)
@require_basic
def api_get_conversationsGroup_for_person_private(people_public_id: int):
    """
    GET /api/v5/people/<people_public_id>/conversationsGroup
...
def api_leave_conversationGroupe_private(conversation_id: int):
    """
    POST /api/v5/conversations/group/<conversation_id>/leave
    Body JSON :
    {
      "people_public_id": 1,
      "soft_delete_own_messages": true,   (optionnel, défaut: true)
...
@ratelimit(5)
@require_basic
def api_get_conversations_for_person_privateUnReadGroup(people_public_id: int):
    with get_session() as session:
        person = session.execute(
            select(PeoplePublic).where(PeoplePublic.id == people_public_id)
        ).scalar_one_or_none()
