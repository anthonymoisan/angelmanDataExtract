...
@require_basic
def auth_login():
    data = request.get_json(silent=True) or request.form.to_dict(flat=True) or request.args.to_dict(flat=True)
...
@ratelimit(10)
@require_basic
def private_update_person_connection():
    """
    Mise à jour privée du statut de connexion d'une personne.
    - Protégée par Basic Auth (require_basic)
    - Attend: person_id, is_connected (bool ou 0/1)
    - Retourne {ok: True, ...} ou 400/404/500
    """
    data = (
    ...
    # --- validation is_connected (accepte bool, 0/1, "true"/"false") ---
    if is_connected in (True, "true", "True", "1", 1):
        is_connected = True
    elif is_connected in (False, "false", "False", "0", 0):
        is_connected = False
    else:
    ...
    try:
        ...
        current_app.logger.exception("Erreur update is_connected")
        return jsonify({"error": "erreur serveur"}), 500

    if not updated:
        return jsonify({"ok": False, "message": "personne introuvable ou inactive"}), 404

    return jsonify({"ok": True, "id": person_id, "is_connected": is_connected}), 200
...
def api_get_secret_question():
    try:
        src = _get_src() or {}
        email = (
            request.args.get("email") or request.args.get("emailAddress")
            or (src.get("email") if isinstance(src, dict) else None)
            or (src.get("emailAddress") if isinstance(src, dict) else None) or ""
        ).strip()
        if not email:
